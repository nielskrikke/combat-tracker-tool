        ...
        <div className="relative" ref={ref}>
            ...
            {isOpen && (
                <div className="absolute z-10 w-full mt-1 bg-gray-600 border border-gray-500 rounded-md shadow-lg max-h-60 overflow-y-auto">
                    {options.map(option => (
                        <label key={option} className="flex items-center px-4 py-2 text-white hover:bg-red-700 cursor-pointer capitalize">
                            <input type="checkbox" checked={selected.includes(option)} onChange={() => onChange(option)} className="h-4 w-4 rounded border-gray-300 text-red-600 focus:ring-red-500" />
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-gray-800 rounded-lg shadow-xl border border-gray-600 w-full max-w-4xl h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                <header className="flex justify-between items-center p-4 border-b border-gray-700">
                    <h2 className="text-2xl font-medieval text-yellow-400">Creature Compendium</h2>
                ...
                <div className="flex-grow flex overflow-hidden">
                    ...
                    <aside className="w-1/3 xl:w-1/4 p-4 border-r border-gray-700 overflow-y-auto space-y-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-400 mb-1">Challenge Rating</label>
                            <div className="flex items-center gap-2">
                                <select value={filters.minCr} onChange={e => handleFilterChange('minCr', e.target.value)} className="w-full bg-gray-700 border border-gray-600 rounded-md px-2 py-2 text-white">
                                    {CR_VALUES.map(cr => <option key={`min-${cr}`} value={cr}>{cr}</option>)}
                                </select>
                                <span className="text-gray-400">to</span>
                                 <select value={filters.maxCr} onChange={e => handleFilterChange('maxCr', e.target.value)} className="w-full bg-gray-700 border border-gray-600 rounded-md px-2 py-2 text-white">
                        ...
                        <label className="flex items-center text-white select-none">
                            <input type="checkbox" checked={filters.isLegendary} onChange={e => handleFilterChange('isLegendary', e.target.checked)} className="h-4 w-4 rounded border-gray-300 text-red-600 focus:ring-red-500"/>
                            <span className="ml-2">Is Legendary</span>
                        </label>

                        <div className="pt-2 flex flex-col gap-2">
                            <button onClick={applyFilters} className="w-full bg-red-700 hover:bg-red-800 text-white font-bold py-2 px-4 rounded-md transition">Apply Filters</button>
                    ...
                    <main className="flex-grow p-4 overflow-y-auto">
                        {isLoading ? (
                            ...
                            <>
                                <p className="text-gray-400 text-sm mb-2">{filteredMonsters.length} of {allMonsters.length} creatures shown</p>
                                {filteredMonsters.length > 0 ? (
                                <ul className="space-y-2">
                                    {filteredMonsters.map(m => (
                                        <li key={m.index}>
                                            <button onClick={() => onSelect(m.url)} className="w-full text-left p-3 bg-gray-700/50 hover:bg-gray-700 rounded-md transition group">
                                                <div className="flex justify-between items-center">
                                                    <p className="font-bold text-white group-hover:text-yellow-400 transition">{m.name}</p>
                                                    <p className="text-sm text-gray-300">CR {crToString(m.challenge_rating)}</p>
                                                </div>
                                                <p className="text-xs text-gray-400 capitalize">{m.size} {m.type}</p>
                                            </button>
                                        </li>
                                    ))}
                                </ul>
